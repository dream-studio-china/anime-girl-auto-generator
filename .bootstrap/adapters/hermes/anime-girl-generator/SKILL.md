...
3. **批量出图** — 使用与日常出图**相同的** execute_code 流程：加载 workflow → 注入10个prompt → 全部提交 → 统一轮询 → 批量下载到 `{PROJECT_DIR}/images/`
4. **交付** — MEDIA: 发送所有图片 + 对应主题名

- **不需要**单独脚本，直接复用日常出图的 execute_code 模板
- 保存路径统一为 `{PROJECT_DIR}/images/`，无子目录
